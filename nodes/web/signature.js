...
async function loadWorkflow(app) {
	try {
		...
		if (get_workflow_data && get_workflow_data.version && get_workflow_data.nodes && get_workflow_data.extra) {
			await app.loadGraphData(get_workflow_data, true, true);
		}	
	} catch (error) {
		showMessage("An Error occurerd while loading the workflow from Signature", "#ff0000ff");
	}
...
async function saveWorkflow(app) {
	try {
		...
		if (response.ok) {
			showMessage("Workflow saved to Signature", "#00ff00ff");
		} else {
			showMessage("An Error occurerd while saving the workflow to Signature", "#ff0000ff");
		}
	} catch (error) {
		showMessage("An Error occurerd while saving the workflow to Signature", "#ff0000ff");
	}
